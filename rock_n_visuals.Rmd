...
#KGA: Check out the changes I made with the GitHub comparison tool
#KGA: #1 Im pretty sure that rather than `TRUE ~ NA_character_` you can just do `TRUE ~ NA`
#KGA: #2 In regards to: sample_name = str_extract(sample_id, ".*(?=-[pn]-)")
#KGA: To me this says that you are looking for a match to all characters, up to (but not including) -p- OR -n-
#KGA: I have modified below for (what I think) is an easier version of this,, or at least I think it is easier to understand what's going on
#KGA: str_extract(sample_id, "(.*)-[pn]-", group = 1)
#KGA: that says "extract the following string from sample_id,, and only extract what is contained in group 1 (the string contained by the parentheses). The pattern is "everything, -(p OR n)-""
...
#KGA: also, i'm not sure what ref_soil is or why it is being removed. is this your control?
```
...
```{r visual 5}
# don't plot mean - boxplot shows mean, silly
