...
resistance <- clean_data |>
  mutate(
    Pd = blows / 0.10,      # blows per meter  (your version: note this is actually inverse Pd, but OK)
    Rs = 202.0 * blows      # resistance per depth increment
  )
...
ggplot(rs_summary,
       aes(x = avg_Rs, y = depth_m, group = pit, color = pit)) +
